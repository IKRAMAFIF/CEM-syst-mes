% tp05_chargee.m - FDTD simulation pour une cavité chargée
...
% MODIFICATION pour le cas chargé : insertion d'un diélectrique
% Propriétés diélectriques: er = 3 dans toutes les directions
% Position: xmin=1m; ymin=1m; zmin=1m / xmax=4m; ymax=6m; zmax=3m
er_dielectric = 3;
x_start_idx = 10; % 1m / 0.1m
x_end_idx = 40;   % 4m / 0.1m
y_start_idx = 10; % 1m / 0.1m
y_end_idx = 60;   % 6m / 0.1m
z_start_idx = 10; % 1m / 0.1m
z_end_idx = 30;   % 3m / 0.1m

EPS_X(x_start_idx:x_end_idx, y_start_idx:y_end_idx+1, z_start_idx:z_end_idx+1) = er_dielectric * eps0;
EPS_Y(x_start_idx:x_end_idx+1, y_start_idx:y_end_idx, z_start_idx:z_end_idx+1) = er_dielectric * eps0;
EPS_Z(x_start_idx:x_end_idx+1, y_start_idx:y_end_idx+1, z_start_idx:z_end_idx) = er_dielectric * eps0;
...
output_file = 'TP3_solution/result_chargee.txt'; % Nom de fichier ajusté
dlmwrite(output_file, Ets, 'delimiter', ' ');
fprintf('Résultats du champ électrique (cavité chargée) sauvegardés dans %s\n', output_file);
